...
The current public release is `v0.1.1` (18 September 2026). The Python 3 MCP adapter and protocol tests are validated on Python 3.10 through 3.13 in GitHub Actions. The default surface exposes 26 MCP tools, with 12 additional trusted-local tools available through `trac-mcp-call`. Every MCP-visible tool declares an `outputSchema` and successful calls return matching `structuredContent` alongside JSON text for backward compatibility. `legacy/trac_broker_py2.py` is the Trac compatibility broker; despite the historical filename, it supports both Python 2 and Python 3 Trac runtimes.
